-- Один безопасный скрипт восстановления платёжной схемы.
-- Можно выполнять даже если уже частично накатили 008/009.
-- Идемпотентно: приводит БД к финальному состоянию без каши.

-- 1) Колонка idempotency (из 008)
alter table public.payments
  add column if not exists idempotency_key text;

-- 2) Уникальный индекс (из 008)
create unique index if not exists idx_payments_org_idempotency
  on public.payments (organization_id, idempotency_key)
  where idempotency_key is not null;

-- 3) Финальная record_payment (версия 009: без переплаты сверх общего остатка)
...
-- 4) Быстрая самопроверка после выполнения
do $$
begin
  if not exists (
    select 1
    ...
    raise exception 'FAIL: нет колонки payments.idempotency_key';
  end if;
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and indexname = 'idx_payments_org_idempotency'
  ) then
    raise exception 'FAIL: нет индекса idx_payments_org_idempotency';
  end if;
  ...
  if not exists (
    ...
    raise exception 'FAIL: нет функции record_payment';
  end if;

  raise notice 'OK: payment schema is consistent';
end $$;
